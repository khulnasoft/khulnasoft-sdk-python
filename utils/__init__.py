...
# Default Khulnasoft cmdline rules
RULES_KHULNASOFT = {
    'config': {
        'flags': ["--config"],
        'action': "callback",
...
FLAGS_KHULNASOFT = list(RULES_KHULNASOFT.keys())
...
def parse(argv, rules=None, config=None, **kwargs):
    """Parse the given arg vector with the default Khulnasoft command rules."""
...
def parser(rules=None, **kwargs):
    """Instantiate a parser with the default Khulnasoft command rules."""
    rules = RULES_KHULNASOFT if rules is None else dict(RULES_KHULNASOFT, **rules)
