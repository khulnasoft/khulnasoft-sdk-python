...
env:
	...
	@echo "To make a .env:"
	@echo "  [KHULNASOFT_INSTANCE_JSON] | python scripts/build-env.py"
...
wait_up:
	...
	@for i in `seq 0 180`; do if docker exec -it $(CONTAINER_NAME) /sbin/checkstate.sh &> /dev/null; then break; fi; printf "\rWaiting for Khulnasoft for %s seconds..." $$i; sleep 1; done
